...
public interface ComplaintRepository extends JpaRepository<Complaint, Long> {
    ...
    @Query("SELECT c FROM Complaint c WHERE c.slaDeadline < :now AND c.status NOT IN ('RESOLVED','CLOSED','REJECTED')")
    List<Complaint> findBreachedComplaints(@Param("now") LocalDateTime now);

    @Query("SELECT c FROM Complaint c WHERE c.slaDeadline BETWEEN :now AND :deadline AND c.status NOT IN ('RESOLVED','CLOSED','REJECTED')")
    List<Complaint> findUpcomingBreaches(@Param("now") LocalDateTime now, @Param("deadline") LocalDateTime deadline);

    @Query("SELECT COUNT(c) FROM Complaint c WHERE c.status = :status")
    long countByStatus(@Param("status") ComplaintStatus status);

    @Query("SELECT COUNT(c) FROM Complaint c WHERE c.assignedAgent = :agent AND c.status NOT IN ('RESOLVED','CLOSED')")
    long countActiveByAgent(@Param("agent") User agent);

    @Query("SELECT c FROM Complaint c WHERE c.createdAt BETWEEN :from AND :to")
    List<Complaint> findByDateRange(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
}
